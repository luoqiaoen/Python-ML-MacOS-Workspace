...
def main():
    ...
    print('Performing logistic regression using different gradient descent')
    ...
    W = np.random.randn(D,10)/28 #denominator sqrt of dimensionality
    b = np.zeros(10)
    LL = []
    lr = 0.0001
    reg = 0.01
    ...
    p_y = forward(Xtest, W, b)
    print "Final error rate:", error_rate(p_y,Ytest)
    print "Elapsed time for Stochastic Gradient Descent:", datetime.now()-t0
    ...
    x2 = np.linspace(0,1,len(LL_stoch))
    plt.plot(x2, LL_stoch, label="stochastic")
